...
pub enum AccountCommand {
    ...
    PlaceOrder {
        ...
        #[clap(
            long,
            help = "Also exit when the position's carry turns negative. Arm it afterwards with arm-order-interest"
        )]
        interest: bool,
        #[clap(
            long,
            help = "Seconds the realized rates are measured over (default 24h, min 1h)"
        )]
        interest_window_seconds: Option<u32>,
        #[clap(
            long,
            help = "Seconds of carry loss the exit may cost (default 14 days)"
        )]
        interest_patience_seconds: Option<u32>,
        #[clap(
            long,
            help = "Annual loss against the lend leg required to arm, in basis points (default: any)"
        )]
        interest_min_negative_apr_bps: Option<u32>,
    },
    /// Anchor an interest trigger's rate measurement (permissionless; re-runnable once a window passes)
    ArmOrderInterest {
        #[clap(long)]
        order: Pubkey,
    },
    ...
    match subcmd {
        ...
        AccountCommand::PlaceOrder {
            ...
            let interest = interest.then(|| InterestTriggerConfig {
                ...
                patience_seconds: interest_patience_seconds,
        ...
        AccountCommand::ArmOrderInterest { order } => {
            processor::marginfi_account_arm_order_interest(&config, order)
        }
