...
impl MarginfiGroupFixture {
    ...
    /// Sets both emode and same-asset leverage caps in one transaction, so no intermediate
    /// state violates the rule that same-asset may not exceed the emode caps.
    #[allow(clippy::too_many_arguments)]
    pub async fn try_update_leverages(
        &self,
        new_admin: Pubkey,
        new_emode_admin: Pubkey,
        new_curve_admin: Pubkey,
