...
/// Warps the clock so interest accrues on the order-tagged banks, then
/// over-drains past the post-accrual max-fee budget. The end gate must
/// reject the over-drain, which relies on `start_execute_order` having
/// accrued the tagged banks before snapshotting.
#[tokio::test]
async fn execute_order_overdraw_after_clock_warp_reverts() -> anyhow::Result<()> {
    // Tight asset/liability margin (`$14 vs $5`, Net ≈ $9). Interest accrual
    // over the clock warp lifts asset_share_value, which gives the keeper a
    // bit more room than the static-time max-fee case, so we over-drain
    // more aggressively (50%) to clearly exceed `(1 - max_fee) × post_accrual_equity`.
    let asset_deposit = 7.0;
    let liability_borrow = 5.0;
    let withdraw_scale = 1.5;
    ...
    let (
        ...
        liability_borrow,
        BankMint::Sol,
        take_profit_trigger(fp!(5.5), slippage_bps(500)),
    )
    .await?;
    assert_active_orders(&borrower_mfi_account_f, 1).await;
    ...
    // Warp the clock forward by 1 day. The tagged banks haven't been touched
    // since the setup deposit/borrow, so they're stale relative to the new
    // slot. Refresh the Pyth Push oracles AFTER advancing so the published
    // timestamp matches whatever clock value bankrun lands on (advance_time
    // may bump past the requested delta to align to a reward interval).
    // end_execute_order's health check covers ALL balances including the
    // uninvolved SOL deposit, so its oracle needs to be fresh too.
    let warp_secs: i64 = 24 * 60 * 60;
    let setup_time = asset_bank_f.load().await.last_update;
    test_f.advance_time(warp_secs).await;
    let new_time = test_f.get_clock().await.unix_timestamp;
    test_f
        .set_pyth_oracle_timestamp(PYTH_USDC_FEED, new_time)
        .await;
    test_f
        .set_pyth_oracle_timestamp(PYTH_SOL_FEED, new_time)
        .await;

    // Sanity: the order-tagged banks haven't auto-accrued — last_update is
    // still at setup time. `start_execute_order` is what advances them.
    assert_eq!(asset_bank_f.load().await.last_update, setup_time);
    assert_eq!(liability_bank_f.load().await.last_update, setup_time);
