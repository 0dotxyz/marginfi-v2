...
use kamino_mocks::kamino_lending::cpi::{
    accounts::RefreshReservesBatch, refresh_obligation, refresh_reserves_batch,
    withdraw_obligation_collateral_and_redeem_reserve_collateral_v2,
};
use kamino_mocks::{
    kamino_lending::cpi::accounts::{
        RefreshObligation, SocializeLossV2FarmsAccounts,
...
pub fn kamino_withdraw_with_refresh<'info>(
    ...
    ctx.accounts.cpi_refresh_reserve_batch()?;
...
#[derive(Accounts)]
pub struct KaminoWithdrawWithRefresh<'info> {
    #[account(
        mut,
        constraint = (
...
impl<'info> KaminoWithdrawWithRefresh<'info> {
    pub fn cpi_refresh_reserve_batch(&self) -> MarginfiResult {
    ...
    pub fn cpi_refresh_obligation(&self) -> MarginfiResult {
        ...
        let cpi_ctx = CpiContext::new(program, accounts)
            .with_remaining_accounts(vec![self.integration_acc_1.to_account_info()]);
...
impl Hashable for KaminoWithdrawWithRefresh<'_> {
    fn get_hash() -> [u8; 8] {
        get_discrim_hash("global", "kamino_withdraw_with_refresh")
    }
}
