...
mod tests {
    ...
    /// (offset 256), followed by the position-transfer fee fields, so a v1-sized account grown
    /// by `resize_global_fee_state` (zero-filled) reads as unset premium config and default fees.
    #[test]
    fn fee_state_v2_field_layout() {
        use std::mem::offset_of;

        assert_eq!(offset_of!(FeeState, premium_wallet), FeeState::V1_LEN);
        assert_eq!(
            offset_of!(FeeState, position_transfer_fee),
            FeeState::V1_LEN + 32
        );
        assert_eq!(
            offset_of!(FeeState, position_transfer_min_value_usd_cents),
            FeeState::V1_LEN + 36
        );
        assert_eq!(offset_of!(FeeState, _reserved0), FeeState::V1_LEN + 40);

        // Zeroed region == premium unset (wallet default => sweeping disabled), default fees
        let fee_state: FeeState = bytemuck::Zeroable::zeroed();
        assert_eq!(fee_state.premium_wallet, Pubkey::default());
        assert_eq!(fee_state.position_transfer_fee, 0);
        assert_eq!(fee_state.position_transfer_min_value_usd_cents, 0);
    }
}
