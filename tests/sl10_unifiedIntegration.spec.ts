...
import {
  ...
  solendIntegrationProtocolMetas,
} from "./utils/integration-instructions";

describe("sl10: Unified integration deposit/withdraw (Solend)", () => {
  ...
  before(async () => {
    user = users[1];
    bank = solendAccounts.get(SOLEND_USDC_BANK);
    market = solendAccounts.get(SOLEND_MARKET);
  ...
  it("(user 1) deposits to the Solend bank via integration_deposit - happy path", async () => {
    ...
    const tx = new Transaction().add(
      ...
      await makeIntegrationDepositIx(
        ...
        depositAmount,
        { solend: {} },
        await solendIntegrationProtocolMetas(
          user.mrgnBankrunProgram,
          {
            bank,
    ...
    // Shares are cTokens; the exchange rate is >= 1, so 0 < shares <= amount
  ...
  it("(user 1) withdraws all from the Solend bank via integration_withdraw - happy path", async () => {
    await refreshPullOraclesBankrun(oracles, bankrunContext, banksClient);
    const [userUsdcBefore, vaultBefore] = await Promise.all([
      getTokenBalance(bankRunProvider, user.usdcAccount),
      getTokenBalance(bankRunProvider, liquidityVault),
