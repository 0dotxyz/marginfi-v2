...
describe("Same-bank deposit", () => {
  ...
  const setBankState = async (
    ...
    const instructions = [
      // Governance alone may restore a paused/reduce-only bank to Operational.
      // This normalizes the predecessor test's state before a fast state change.
      await configureBank(groupAdmin.mrgnProgram, {
        bank: bankKeypairA.publicKey,
        bankConfigOpt: {
          ...blankBankConfigOptRaw(),
          operationalState: { operational: undefined },
        },
      }),
    ];
